...
el_blockhead_t *el_block_below(el_blockhead_t *block){
  if((void *) block - sizeof(el_blockfoot_t) <= el_ctl->heap_start){
    return NULL;
  }
  ...
  if((void *) lower <= el_ctl->heap_start){
    return NULL;
  }
  else{
    return lower;
...
el_blockhead_t *el_split_block(el_blockhead_t *block, size_t new_size){
  if(block->size >= new_size + EL_BLOCK_OVERHEAD){
    ...
    el_blockfoot_t *block_foot = PTR_PLUS_BYTES(block, new_size + sizeof(el_blockhead_t));
    block_foot->size = new_size;

    el_blockhead_t *new_header = PTR_PLUS_BYTES(block, new_size + EL_BLOCK_OVERHEAD);
    new_header->size = start - EL_BLOCK_OVERHEAD - new_size;
    new_header->state = EL_AVAILABLE;

    el_blockfoot_t *new_foot = el_get_footer(new_header);
    new_foot->size = start - EL_BLOCK_OVERHEAD - new_size;
...
void *el_malloc(size_t nbytes){
  el_blockhead_t *memblock = el_find_first_avail(nbytes);
  if(memblock == NULL){
    return NULL;
  } 
  el_remove_block(el_ctl->avail, memblock);
  el_blockhead_t *split = el_split_block(memblock, nbytes);      
  // el_remove_block(el_ctl->avail, memblock);
  el_add_block_front(el_ctl->avail, split);
  el_add_block_front(el_ctl->used, memblock);
  

  return PTR_PLUS_BYTES(memblock, sizeof(el_blockhead_t));
}
...
void el_merge_block_with_above(el_blockhead_t *lower){
  ...
  if((below == NULL || below->state != EL_AVAILABLE) && (higher == NULL || higher->state != EL_AVAILABLE)){
  ...
  if(below != NULL && below->state == EL_AVAILABLE){
    el_remove_block(el_ctl->used, below);
    if(lower != NULL && lower->state == EL_AVAILABLE){
      el_remove_block(el_ctl->used, lower);
      el_blockfoot_t *foot = el_get_footer(lower);
      lower->size += (below->size + EL_BLOCK_OVERHEAD);
      foot->size = lower->size;
      lower = PTR_MINUS_BYTES(lower, below->size + EL_BLOCK_OVERHEAD);
    }
  }
  if(higher != NULL && higher->state == EL_AVAILABLE){
    el_remove_block(el_ctl->used, higher);
    if(lower != NULL && lower->state == EL_AVAILABLE){
      el_remove_block(el_ctl->used, lower);
...
void el_free(void *ptr){
  ...
  if(head == NULL || head->state != EL_USED){
    return;
  }
  if(head != NULL){
    head->state = EL_AVAILABLE;
    // since head != NULL && head->state == EL_AVAILABLE, el_merge should merge and upate the lists
